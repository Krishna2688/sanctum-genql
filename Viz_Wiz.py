...
def load_history():
    if st.session_state['viz_active_conversation']:
        for idx, m in enumerate(st.session_state["viz_messages"]):
            print(f"ch {idx}")
            for r, c in m.items():
                if r == "role":
                    role = c
                    ...
                    if role == "user":
                        ...
                        if len(st.session_state["chart"]) > 0:
                            assistant_message.plotly_chart(st.session_state["chart"][str(idx+1)])
...
model_options = ["llama3.2", "OpenAI (GPT-3.5)", "codellama", "gemini-1.5-flash-latest", "gemini-2.0-flash-exp"]
...
# if chat_button:
nl_query = st.chat_input("Describe the visualization (e.g., 'Total sales per product')", disabled=chat)
...
if nl_query:
    ...
    if st.session_state['viz_active_conversation']:
        if st.session_state["viz_model_change"]:
            for idx, m in enumerate(st.session_state["viz_messages"][last:]):
                print(f"ch {m}")
                for r, c in m.items():
                    if r == "role":
                        role = c
                        ...
                        if role == "user":
                            ...
                            assistant_message.write(c)
                            if len(st.session_state["chart"]) > 0:
                                assistant_message.plotly_chart(st.session_state["chart"][idx])
    ...
    with st.spinner("Thinking..."):
        if st.session_state["viz_model"]:
            ...
            elif st.session_state["viz_model"].startswith("gemini"):
    ...
    exec_namespace = {}
    ai_response = re.sub(r"```(?:python)?\n?|```", "", response).strip()
    with st.spinner("generating viz.."):
        time.sleep(5)
        try:
            exec(ai_response, {"pd": pd, "px": px, "st": st}, exec_namespace)
            st.success("✅ Visualization generated successfully!")
            with st.expander("Viz"):
                assistant_message.plotly_chart(exec_namespace["fig"])
                print(f"len {len(st.session_state['viz_messages'])}")
                st.session_state["chart"][str(len(st.session_state["viz_messages"]))] = exec_namespace["fig"]
        except Exception as e:
            st.error(f"❌ Error executing AI-generated code: {e}")
            # with st.expander("Viz"):
            #     assistant_message.error(f"❌ Error executing AI-generated code: {e}")
            # st.plotly_chart()
